...
- **CSO-F2-aanvulling (2026-08-10):** de onafhankelijke CSO-review vond dat de fail-closed-fix alleen "store down" dekte, niet "store nooit geconfigureerd in prod" (ontbrekende/ver-typte `UPSTASH_*` → guard inert → onbeperkte anonieme Opus-5 in launch-mode). `consumeAnalysisBudget` (WIP `counters.ts`) faalt nu ook CLOSED wanneer de store niet-geconfigureerd is én `NODE_ENV==="production"`; alleen lokaal/dev blijft open. tsc schoon. (Zit in dezelfde untracked counter-store-WIP — commit store+guard sámen.) Overweeg daarnaast `DAILY_ANALYSIS_BUDGET`-default te verlagen: de gedocumenteerde €30/dag is voor 8-frame Opus-5-vision 3-5× te optimistisch (CSO-R2).
