...
- **Severity:** Medium · **Status:** GEFIXT (2026-08-09, kern) — de daily-budget-guard faalt niet langer OPEN bij een store-outage: `consumeAnalysisBudget` geeft nu een `unavailable`-state terug wanneer de store **geconfigureerd maar onbereikbaar** is (via nieuwe `storeConfigured()`), en `budgetGuard` antwoordt dan **503** i.p.v. de LLM-call door te laten. Lokaal/dev (geen store-config) blijft ongewijzigd open. Alleen de 3 betaalde AI-routes (room/analyze, spots/analyze, room/ask) gebruiken de guard; `roof/analyze` is PDOK open-data (geen LLM). Typecheck schoon; 9 store-tests groen. **Optioneel vervolg (niet-blokkerend):** per-IP rate-limit + captcha als extra laag bovenop de globale dagcap; auth-gate in prod verplicht aanzetten (runtime-besluit, S-001-not-verified).
